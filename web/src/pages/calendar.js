...
export const Head = () => {
  return (
    <>
      <title>Milwaukee Tech Meetups</title>
      <meta
        name="description"
...
export default function CalendarPage() {
  ...
  useEffect(() => {
    const organizationData = JSONData.sort((a, b) =>
  ...
  const handleEventClick = (clickInfo) => {
    ...
    const groups = sortedByDays.findIndex((a) =>
      compareDates(new Date(a[0].startDate), new Date(date))
    );
        ...
        if (sortedEvents.length === 0) {
          ...
          while (counter <= sortedEvents.length - 1 && !dayFound) {
            const comparison = compareDates(
              sortedEvents[index][0].startDate,
              b.startDate
            );
  ...
  return (
    <PageLayout>
      ...
      <section className={styles.pageContainer}>
        ...
        <section
          className={`srcryBox ${styles.calPage} ${styles.calContainer}`}
        >
          <FullCalendar
