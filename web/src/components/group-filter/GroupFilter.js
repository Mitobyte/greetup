...
export const GroupFilter = ({ nameList, selected, toggle, resultList }) => {
  ...
  const handleSelection = (name) => {
    name === 'all'
      ...
      : selectedNames.indexOf('all') > -1
        ? setSelectedNames([name])
        : selectedNames.indexOf(name) > -1 && selectedNames.length === 1
          ? setSelectedNames(['all'])
  ...
  const handleUserInput = (input) => {
    const groups = nameList.filter((a) =>
  ...
  return (
    <article className={styles.mainContainer}>
      ...
      <article className={styles.searchBar}>
        <SearchBar userInput={handleUserInput} searchText={handleSearch} />
      </article>
